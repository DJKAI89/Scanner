...
export function buildConfidenceRows(p, kind) {
  ...
  if (kind === 'option') {
    ...
    if (p.confluence != null) rows.push({ label: 'Confluence Score', val: (p.confluence >= 0 ? '+' : '') + p.confluence, dir: Math.sign(p.confluence) });
  ...
  if (p.mlProbability != null) {
    rows.push({ label: 'AI Model Probability', val: Math.round(p.mlProbability * 100) + '%' + (p.aiModel ? ` (${p.aiModel})` : ''), dir: p.mlAdj > 0 ? 1 : p.mlAdj < 0 ? -1 : 0 });
  }
